...
/** Render a string with **bold** markers */
...
/** Split content string into paragraphs and render bold markers */
          ...
          return (
            ...
            >
              ...
              <div className="mt-2 text-[10px] text-white/40">
                {row.lifespan}-year lifespan · ${(row.upfront / row.lifespan).toFixed(0)}/yr effective
              </div>
            </motion.div>
          )
        })}
...
export default function BlogPost() {
  ...
  // Scroll to top on mount / postId change
  useEffect(() => {
    window.scrollTo(0, 0)
  }, [postId])
  ...
  useEffect(() => {
    if (post) {
      document.title = `${post.title} | ${company.shortName}`
    } else {
      document.title = `Post Not Found | ${company.shortName}`
    }
  }, [post])
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 pb-12 sm:pb-16 w-full">
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-4 text-white/50 text-xs">
              ...
              {post.relatedService && (
                <Link
                  to={`/services/${post.relatedService}`}
                  className="flex items-center gap-1.5 hover:text-accent transition-colors"
                >
                  <ChevronRight className="w-3.5 h-3.5 text-accent" />
                  View {post.relatedService.replace(/-/g, ' ')} service
                </Link>
              )}
            </div>
          </motion.div>
        ...
        <div className="max-w-4xl mx-auto px-6 py-10 sm:py-14">
          ...
          {post.tags && post.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-8 pb-8 border-b border-white/10">
              {post.tags.map((tag) => (
                <span
                  key={tag}
                  className="text-[10px] font-medium text-white/40 bg-white/5 px-2.5 py-1 rounded-full"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="mt-16 pt-8 border-t border-white/10">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <p className="text-white/40 text-xs uppercase tracking-widest mb-1">Published by</p>
                <p className="text-white font-bold text-sm">{company.shortName}</p>
                <p className="text-white/40 text-xs">{formatDate(post.publishDate)}</p>
              </div>
              {post.relatedService && (
                <Link
                  to={`/services/${post.relatedService}`}
                  className="inline-flex items-center gap-2 bg-accent/10 hover:bg-accent/20 text-accent font-bold text-sm px-5 py-2.5 rounded-lg ring-1 ring-accent/30 transition-colors"
                >
        ...
        <section className="bg-surface-alt py-16 sm:py-20">
          <div className="max-w-4xl mx-auto px-6">
            ...
            <div className="text-center mt-10">
              <Link
                to="/blog"
                className="inline-flex items-center gap-2 text-white/50 hover:text-accent text-sm font-medium transition-colors"
              >
                <ArrowLeft className="w-4 h-4" />
                All Articles
              </Link>
